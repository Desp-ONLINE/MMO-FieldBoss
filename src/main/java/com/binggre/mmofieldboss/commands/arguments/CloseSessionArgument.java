...
public class CloseSessionArgument implements CommandArgument {
    ...
    public boolean execute(@Nonnull CommandSender sender, @Nonnull Command command, @Nonnull String s, @Nonnull String[] args) {
        ...
        try {
            data.spawn();
            sender.sendMessage("§a입장 마감 + 보스 스폰 완료 (보스 ID: " + id + ")");
            return true;
        } catch (InvalidMobTypeException e) {
            e.printStackTrace();
            sender.sendMessage("§c스폰 실패: " + e.getMessage());
            return false;
        }
    }
    ...
    public String getDescription() {
        return " [ID] - 입장을 마감하고 보스를 스폰합니다.";
    }

    @Override
    public String getPermission() {
        return "mmofieldboss.admin.closesession";
    }
